...
for (i in 1:50){
  ...
  mse.svr.U.class.vec = sapply(1:3, function(ix) mean((Yhat.svr.U.class[[ix]] - Y.test[[ix]])^2))
  ...
  mse.svr.U.vec = sapply(1:3, function(ix) mean((Yhat.svr.U.test[[ix]]-Y.test.list[[ix]])^2))
...
mse.U.vec.result = array(as.numeric(unlist(mse.U.vec.list)), dim=c(8, 3, 50))
mse.U.vec.result = aperm(mse.U.vec.result, c(3, 1, 2))


save(mse.U.result, mse.U.vec.result, file = "X2U4_0.RData")


(1:50)[mse.X.result[,4]>mse.U.result[,8]]
####
