...
for (i in 1:50){
  ...
  # mycut4 = X2U4(X.train.list, plot = T)
  ...
  mse.U.list[[i]] = mse.U
  mse.mean.vec.list[[i]] = mse.mean.vec
}
...
mse.mean.vec.result = do.call(rbind, mse.mean.vec.list)

save(mse.U.result, mse.U.vec.result, mse.mean.vec.result, file = "X2U3.RData")
