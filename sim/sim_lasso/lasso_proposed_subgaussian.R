...
for (s in seq(0, 5, by = 0.1)){
  ...
  for (ii in 1:50){
    F1 = rmvt(n, diag(K), df = t.df)
    F2 = rmvt(n, diag(K), df = t.df)
    F3 = rmvt(n, diag(K), df = t.df)
    
    F1.train = F1
    F2.train = F2
    F3.train = F3
    
    U1 = rmvt(n, para1$SigmaU, df = t.df)
    U2 = rmvt(n, para2$SigmaU, df = t.df)
    U3 = rmvt(n, para3$SigmaU, df = t.df)
    ...
    eps1 = rmvt(n, sigma = matrix(sigma.vec[1]), df = t.df)
    eps2 = rmvt(n, sigma = matrix(sigma.vec[2]), df = t.df)
    eps3 = rmvt(n, sigma = matrix(sigma.vec[3]), df = t.df)
  ...
  file.name = paste0("result_lasso_subgaussian_s=", format(s, nsmall = 1), ".RData")
