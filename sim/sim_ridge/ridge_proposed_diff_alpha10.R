...
for (ii in 1:10){
  ...
  ml = ridge.OLS.U
  beta.alpha = coef(ml, s=ml$lambda.min)[-1]
  beta1.group = coef(ml.lasso.X.class[[1]], s=ml.lasso.X.class[[1]]$lambda.min)[-1]
  beta2.group = coef(ml.lasso.X.class[[2]], s=ml.lasso.X.class[[2]]$lambda.min)[-1]
  beta3.group = coef(ml.lasso.X.class[[3]], s=ml.lasso.X.class[[3]]$lambda.min)[-1]
  beta.global = coef(ml.lasso.global, s=ml.lasso.global$lambda.min)[-1]
  ...
  mse.alpha.list = compute.mse(Y.test.list, Yhat.test.ridge.OLS.list)
  
  # --------------------------------------- ALPHA-10 ---------------------------------------------
  X2U.list = lapply(1:n_label, function(ix) X2U2(X.train.list[[ix]], K = 10, plot = F))
  H.list = lapply(X2U.list, function(list) list$H)
  K.list = lapply(X2U.list, function(list) list$K)
  P.list = lapply(X2U.list, function(list) list$P)
  L.list = lapply(X2U.list, function(list) matrix(list$L[-1,], ncol = p)) 
  ...
  PYhat.test.list = lapply(1:n_label, function(ix) F.test.list[[ix]]%*%ml.lm.F.list[[ix]]$coefficients)
  mse.alpha10.list = compute.mse(Y.test.list, PYhat.test.list)
  ...
  Yhat.test.ridge.OLS.list = lapply(1:n_label, function(ix) PYhat.test.list[[ix]] + HYhat.test.ridge.OLS.list[[ix]])
  mse.alpha0.list = compute.mse(Y.test.list, Yhat.test.ridge.OLS.list)
  
  mse = rbind(mse.lasso.global.vec, mse.lasso.X.class.vec, mse.alpha0.list[[1]], mse.alpha10.list[[1]], mse.alpha.list[[1]])
  row.names(mse) = NULL
