...
for (s in seq(0, 40, length.out = 25)){
  print(s)
  for (ii in 1:80){
    beta1_unique = c(1, 1, -1)*s/sqrt(p)
    beta2_unique = c(1, -1, 1)*s/sqrt(p)
    beta3_unique = c(-1, 1, 1)*s/sqrt(p)
    ...
    print(c(mse.lasso.global, mse.lasso.X.class, mse.ridge.OLS.list$mse))
    DD = rbind(DD, c(mse.lasso.global, mse.lasso.X.class, mse.ridge.OLS.list[[2]]))
  }
}
